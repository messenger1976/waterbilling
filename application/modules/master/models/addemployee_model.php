...
class addemployee_model extends CI_Model {
    ...
    public function get_single_record($id='') {
        $this->db->select("*,(Select job_title from ".$this->table_job_title." where ".$this->table_job_title.".id = ".$this->table_name.".job_title) as jobtitle,(Select account_name  from ".$this->table_account." where ".$this->table_account.".id = ".$this->table_name.".account_id) as subName");
		$this->db->from($this->table_name);
		if($id != ''){
			$this->db->where("id",$id);
			$query = $this->db->get();
	...
	public function add_record(){
		...
		//$employee_id  = 'WT'.$brcE;
		$set_data = array(
		                  'account_id' =>$this->input->post('acount_group'),
